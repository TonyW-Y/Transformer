...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        ...
        if lang == 'zh':
            # Chinese has no whitespace between words, so Whitespace() would treat
...
def get_ds(config):
    # opus-100 has 1M sentence pairs (opus_books-scale tutorials assume ~30-100k),
    # so cap it to a comparable size to keep training tractable on a single machine
    ds_raw = load_dataset(f"{config['datasource']}", f"{config['lang_src']}-{config['lang_tgt']}", split='train[:30000]')

    # Build tokenizers
    tokenizer_src = get_or_build_tokenizer(config, ds_raw, config['lang_src'])
    tokenizer_tgt = get_or_build_tokenizer(config, ds_raw, config['lang_tgt'])

    # Drop pairs that won't fit in seq_len (BilingualDataset raises on these otherwise)
    ...
    ds_raw = ds_raw.filter(fits_seq_len)
